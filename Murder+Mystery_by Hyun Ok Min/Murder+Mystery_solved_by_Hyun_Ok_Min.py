...
def get_average_sentence_length(text):
    ...
    sentences_in_text = text_replace.split('.')
    pos = 'out'
    sentence_lengths = 0
    for word in text:
        if word != ' ' and pos == 'out':
            ...
            pos = 'out'
    average_sentence_lengths = sentence_lengths / (len(sentences_in_text) - 1)
...
murderer_sample = TextSample(murder_note, "The Murder")
...
#i.e. text = "Where did you go, friend? We nearly saw each other."

def prepare_text(text):
    lowered_text = text.lower()
    replaced_text = lowered_text.replace("?", ".").replace("!", ".").replace(",", " ").replace("'", " ")
    sentences_in_text = remove_punctuation(replaced_text).split()
...
# The prepared text is saved as self.prepared_text under TextSample class above.
...
#i.e. corpus = ['do', 'you', 'see', 'what', 'i', 'see']
...
# The word frequency table is assigned to self.word_count_frequency under TextSample class above.
...
#i.e. ngram_creator = ['what', 'in', 'the', 'world', 'is', 'going', 'on']
...
print(percent_difference(22.066666666666666, 15.842105263157896))
...
print("\nLily Trebuchet: "+"\nScore: "+str(find_text_similarity(murderer_sample, Lily_sample))+"\n")
print("\nMyrtle Beech: "+"\nScore: "+str(find_text_similarity(murderer_sample, Myrtle_sample))+"\n")
print("\nGregg T. Fishy: "+"\nScore: "+str(find_text_similarity(murderer_sample, Gregg_sample)))
